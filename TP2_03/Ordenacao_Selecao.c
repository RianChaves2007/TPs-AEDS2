...
#include <string.h>
#include <stdbool.h>
#include <time.h> // para calcular o tempo de execução do algorítmo de ordenação

/** Observações:
 * 
 * Para entender melhor a parte de tipos de dados e a protótipação 
 * das funções deste arquivo, vide o código: 
 * ColecaoRestaurantes.c que estará disponível em: 
 * https://github.com/RianChaves2007/TPs-AEDS2.git
 *
 * A partir da sessão identificada como "ORDENACAO  POR  SELECAO" 
 * (antes da main) neste código, estarão todas as funções criadas 
...
void selecao(Restaurante** array, int n, long long* mov,long long* comp) {
    for (int i = 0; i < n - 1; i++) {
        int menor = i;
        for (int j = i + 1; j < n; j++) {
            (*comp)+=1;
...
int main() {
    ...
    long long mov = 0, comp = 0; // long long garante 64 bits de precisão.

    Restaurante** array = (Restaurante**) malloc(col->tamanho * sizeof(Restaurante*));
    int n_rest = 0; // número de restaurantes que serão ordenados
    int id;
    ...
    clock_t ini = clock();
    selecao(array, n_rest, &mov, &comp);
    clock_t fim = clock();
    double tempo = (double)(fim - ini) / CLOCKS_PER_SEC;

    char buffer[1024];
    for (int i = 0; i < n_rest; i++) {
        formatar_restaurante(array[i], buffer);
        printf("%s\n", buffer);
    }

    FILE* log = fopen("898910_selecao.txt", "w");
    if (log) {
